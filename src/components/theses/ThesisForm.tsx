...
import { addThesis, updateThesis } from "@/lib/api";
...
interface ThesisFormProps {
  initialData?: Thesis & { author_name?: string }; // author_name for initialData for edit
  universities: University[];
  specializations: Specialization[];
  degrees: Degree[];
}

export function ThesisForm({ initialData, universities, specializations, degrees }: ThesisFormProps) {
  ...
  const defaultValues = initialData
    ? {
        ...
        author_name: initialData.author?.name || initialData.author_name || "", // Handle author_name from potential AddThesisResponse structure
      }
    : {
        title: "",
  ...
  async function onSubmit(data: ThesisFormValues) {
    ...
    try {
      ...
      router.push("/theses");
      router.refresh(); // To see changes in the table
    } catch (error: any) {
      toast({
        title: "خطأ",
